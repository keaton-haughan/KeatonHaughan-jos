...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
    ...
    if ((uintptr_t)srcva < UTOP) {
        ...
        // Use PTE_SYSCALL as the allowed permission mask, same as sys_page_alloc
        ...
        if ((uintptr_t)dst->env_ipc_dstva < UTOP) {
            if (page_insert(dst->env_pgdir, pp, dst->env_ipc_dstva, perm) < 0)
                return -E_NO_MEM;
...
sys_ipc_recv(void *dstva)
{
    ...
    curenv->env_tf.tf_regs.reg_eax = 0;  // return 0 when resumed
